...
my $base_length = int($sample_rate / 8);
...
sub perl_render {
	...
	for ($i = 0; $i < $size; $i++) {
		...
		if ($trigger < $frame) {
			...
			$triggerl = $base_length * ( int(rand(2)) + 1 );
		...
		# downstair note > sine
		if (0 == $frame % ($base_length*2)) {
			$f5 = OtoPerl::Basic::freq_bynote(OtoPerl::Basic::scalednote([0,4,5,11,7], 48 - (($frame / ($base_length*2)) % 24) ) );
		}
		my $v5 = 0.8 * OtoPerl::Basic::osc_sin( $frame, $f5 );

		# upstair note > square > (sine LFO > AM)
		if (0 == $frame % ($base_length*4)) {
			$f6 = OtoPerl::Basic::freq_bynote(OtoPerl::Basic::scalednote([0,5,4,7,11], ($frame / ($base_length*4)) % 16 + 10));
		}
